...
-- MD-010 invalidates all legacy prune decisions until canonical admission has
-- been re-evaluated under a versioned admission policy.
...
-- Defaults above exist only to backfill v0.1.1 rows safely. New MD-010 writes
-- must explicitly declare the policy and curator that produced the receipt.
ALTER TABLE memory_distillation_receipts
