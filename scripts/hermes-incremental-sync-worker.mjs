...
try {
 ...
 const result=await sync.runOnce(); console.log(`DLMF_HERMES_INCREMENTAL=PASS scanned=${result.scanned} changed=${result.changed} ingested=${result.ingested} unchanged=${result.unchanged}`); await runtime.close();
