...
The separately invoked insight-promotion operator provides checksum-bound Plan,
Dry-run, and Apply modes. Apply requires a sealed same-life approval manifest,
revalidates canonical evidence closure and exact scoped state, and verifies the
append-only audit sequence. Production migration plans also bind a protected
mode-`0600` schema backup. Neither workflow enables automatic promotion or pruning;
see [DLMF-SG-010](docs/dlmf-sg-010-production-insight-promotion-operator.md).
